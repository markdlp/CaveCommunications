...
def checkcollision(packet):
    ...
    processing = 0
    for i, _ in enumerate(packetsAtBS):
        if packetsAtBS[i].packet.processed == 1:
            processing = processing + 1
    if (processing > maxBSReceives):
        print("too long:", len(packetsAtBS))
        packet.processed = 0
    else: packet.processed = 1

    if packetsAtBS:
        print(f"CHECK node {packet.nodeid} (sf:{packet.sf} bw:{packet.bw} freq:{packet.freq:.6e}) others: {len(packetsAtBS)}")
        for other in packetsAtBS:
            if other.nodeid != packet.nodeid:
                print(f">> node { other.nodeid} (sf:{other.packet.sf} bw:{other.packet.bw} freq:{other.packet.freq:.6e})")
                # simple collision
...
def sfCollision(p1, p2):
    if p1.sf == p2.sf:
        print(f"collision sf node {p1.nodeid} and node {p2.nodeid}")
...
def powerCollision(p1, p2):
    powerThreshold = 6 # dB
    print(f"pwr: node {p1.nodeid} {p1.rssi:3.2f} dBm node {p2.nodeid} {p2.rssi:3.2f} dBm; diff {round(p1.rssi - p2.rssi,2):3.2f} dBm")
    if abs(p1.rssi - p2.rssi) < powerThreshold:
        print(f"collision pwr both node {p1.nodeid} and node {p2.nodeid}")
